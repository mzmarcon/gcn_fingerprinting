...
class ACERTA_reading_ST(Dataset):
    def __init__(self, split=0.8, condition='none', adj_threshold=0.0, window_t=320, prune=False, binary_adj=False):
        ...
        # csv_file = csv_file.drop(to_remove)
...
class ACERTA_regression_ST(Dataset):
    """
    Dataset class for dyslexia classification
    """
    def __init__(self, split=0.8, condition='none', adj_threshold=0.0, window_t=300, minmax=False, binary_adj=False):
        self.split = split
        ...
        ambac_cn_data_file = data_path + 'shen_cn_task_AMBAC.hdf5'
        psc_ambac_file = hdf5_path + 'shen_psc_task_AMBAC.hdf5'
        schools_labels = data_path + 'schools_accuracy_labels.csv'
        ambac_labels = data_path + 'ambac_accuracy_labels.csv'
        dyslexic_labels = data_path + 'dyslexic_labels.csv'
        stimuli_path = data_path + 'stimuli2/'
        ...
        schools_csv_file = pd.read_csv(schools_labels)
        ambac_csv_file = pd.read_csv(ambac_labels)
        dyslexic_csv_file = pd.read_csv(dyslexic_labels)

        schools_df = schools_csv_file[schools_csv_file['total_accuracy']>0]
        ambac_df = ambac_csv_file[ambac_csv_file['total_accuracy']>0]
        
        schools_df = schools_df[schools_df['ids'].isin(dyslexic_csv_file['id'].values)]
        ambac_df = ambac_df[ambac_df['ids'].isin(dyslexic_csv_file['id'].values)]

        schools_ids = schools_df['ids'].tolist()
        ambac_ids = ambac_df['ids'].tolist()
        schools_labels = schools_df['total_accuracy'].tolist()
        ambac_labels = ambac_df['total_accuracy'].tolist()

        self.ids = schools_ids + ambac_ids
        labels = np.array(schools_labels + ambac_labels).reshape(-1,1)
