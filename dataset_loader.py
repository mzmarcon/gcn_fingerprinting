...
class ACERTA_data(Dataset):
    ...
    def process_betas_irreg_dataset(self,file_rst,file_task,ids,labels_dict):
        dataset = []
        adj_rst = self.generate_mean_adj(file_rst,ids)
...
class ACERTA_condition(Dataset):
    def __init__(self, set, split=0.8, transform=None):
        data_path = './'
        rst_data_file = data_path + 'rst_cn_data.hdf5'
        # task_data_file = data_path + 'task_data.hdf5'
        ...
        if set == 'training':
            self.dataset = self.process_betas_irreg_dataset(file_rst,file_task,train_ids,labels_dict)
            
        if set == 'test':
            self.dataset = self.process_betas_irreg_dataset(file_rst,file_task,test_ids,labels_dict)
        ...
        for n_anchor in range(len(dataset)): #get matching example indices for each example
            ...
            for n_pair in range(len(dataset)):
                if not n_anchor == n_pair:  #remove self loops
                    if dataset[n_anchor]['graph']['id'][0] ==  dataset[n_pair]['graph']['id'][0]:
